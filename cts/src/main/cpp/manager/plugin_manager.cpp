...
static void *foo(void *foo) {
    PluginManager *pluginManager_ = (PluginManager *)foo;
    pluginManager_->runner_->start();
    return NULL;
}

napi_value PluginManager::NapiStartRunner(napi_env env, napi_callback_info info)
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "PluginManager", "NapiStartRunner");
    if ((env == nullptr) || (info == nullptr)) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "PluginManager", "NapiStartRunner: env or info is null");
        return nullptr;
    }
    napi_value thisArg;
    if (napi_get_cb_info(env, info, nullptr, nullptr, &thisArg, nullptr) != napi_ok) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "PluginManager", "NapiStartRunner: napi_get_cb_info fail");
        return nullptr;
    }
    ...
    // Create a new thread
    pthread_t thread_id;
    if (pthread_create(&thread_id, NULL, foo, (void *)pluginManger) != 0) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "PluginManager", "NapiStartRunner: Failed to create thread");
    }    
 
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "PluginManager", "cts thread started");
